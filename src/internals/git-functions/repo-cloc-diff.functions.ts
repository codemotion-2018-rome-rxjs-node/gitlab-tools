...
export function commitDiffPairs(commitsByMonth: { [yearMonth: string]: CommitCompact[] }) {
    ...
    const pairs: { [yearMonth: string]: CommitPair } = {}
    for (let i = 0; i < yearMonths.length - 1; i++) {
        ...
        const newPair: CommitPair = leastRecentCommit ? [mostRecentCommit, leastRecentCommit] : null
    ...
    const pairFirstMonth: CommitPair = firstMonthCommits.length > 1 ?
...
export function reposCommitsPairsDiff(reposCommits: { [repoPath: string]: { [yearMonth: string]: CommitCompact[] } }) {
    const reposCommitsPairs: { repoPath: string, commitPairs: { [yearMonth: string]: CommitPair } }[] = []
