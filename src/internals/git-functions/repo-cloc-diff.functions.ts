...
export function commitDiffPairs(commitsByMonth: { [yearMonth: string]: CommitCompact[] }) {
    ...
    const pairs: { [yearMonth: string]: [CommitCompact, CommitCompact] | null } = {}
    for (let i = 0; i < yearMonths.length - 1; i++) {
        ...
        const newPair: [CommitCompact, CommitCompact] | null = leastRecentCommit ? [mostRecentCommit, leastRecentCommit] : null
    ...
    const pairFirstMonth: [CommitCompact, CommitCompact] | null = firstMonthCommits.length > 1 ?
...
// // #copilot - this function has been generated by copilot after I have written the name of the function
// function checkPairTimeOrder(
//     pair: [CommitCompact, CommitCompact] | null,
//     leastRecentCommitInPreviousMonth: CommitCompact | null
// ) {
//     if (!pair) {
//         return
//     }
//     const mostRecentCommit = pair[0]
//     const leastRecentCommit = pair[1]
//     if (mostRecentCommit.date < leastRecentCommit.date) {
//         throw new Error("The most recent commit must be more recent than the least recent commit")
//     }
//     if (!leastRecentCommitInPreviousMonth) {
//         return
//     }
//     if (leastRecentCommitInPreviousMonth.date < mostRecentCommit.date) {
//         throw new Error("The most recent commit must be more recent than the least recent commit")
//     }
// }